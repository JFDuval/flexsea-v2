...
{
	//Simple test: if we pack a short payload (text string), do we get the correct length (including the \0) + 3 bytes for the header/footer/checksum?
	...
	uint8_t ret_val = comm_pack_payload(payload, payload_len, packed_payload, &packed_payload_len);
	TEST_ASSERT_EQUAL_MESSAGE(0, ret_val, "comm_pack_payload() is reporting an error");
	TEST_ASSERT_EQUAL_MESSAGE(payload_len + 3, packed_payload_len, "How many bytes does generating a string add?");
...
{
	//We pack 48 chars, knowing that with overhead it will be too long. It should return  an error, and no packed payload.
	uint8_t payload[48] = {[0 ... 47] = 'x'};
	uint8_t payload_len = sizeof(payload);
	uint8_t packed_payload_len = 0;
	uint8_t packed_payload[PACKAGED_PAYLOAD_LEN] = {0};
	uint8_t ret_val = comm_pack_payload(payload, payload_len, packed_payload, &packed_payload_len);
	TEST_ASSERT_NOT_EQUAL_MESSAGE(0, ret_val, "comm_pack_payload() is not reporting an error when it should");
...
void test_comm_pack_payload_escape(void)
{
	...
	uint8_t ret_val = comm_pack_payload(payload, payload_len, packed_payload, &packed_payload_len);
	TEST_ASSERT_EQUAL_MESSAGE(0, ret_val, "comm_pack_payload() is reporting an error");
	TEST_ASSERT_EQUAL_MESSAGE(payload_len + 4, packed_payload_len, "How many bytes does generating a string add?");
...
void test_comm_unpack_payload_simple(void)
{
	...
	ret_val = comm_unpack_payload(&cb, extracted_packed_payload,extracted_unpacked_payload);
	if(ret_val)
	{
	...
	TEST_ASSERT_EQUAL_UINT8_ARRAY(packed_payload, extracted_packed_payload, packed_payload_len + 1);
	TEST_ASSERT_EQUAL_UINT8_ARRAY(payload, extracted_unpacked_payload, payload_len + 1);
}
